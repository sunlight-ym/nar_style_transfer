...
class Field(RawField):
	...
	def __init__(self, sequential=True, use_vocab=True, 
					...
					dtype=torch.long, batch_first=False, additional_special_tokens=[], skip_str2index=False):
		...
		self.skip_str2index = skip_str2index
	...
	def pad(self, minibatch, fix_length=None, truncate_first=False, use_init=False, 
		use_eos=False, reverse=False, include_lengths=False,
		...
		pad_idx = [self.vocab.stoi[self.pad_token] if self.use_vocab else self.pad_token] if pad_value is None else [pad_value]
		if use_init:
			assert (self.init_token is not None) or (init_value is not None)
			init_idx = [self.vocab.stoi[self.init_token] if self.use_vocab else self.init_token] if init_value is None else [init_value]
		if use_eos:
			assert (self.eos_token is not None) or (eos_value is not None)
			eos_idx = [self.vocab.stoi[self.eos_token] if self.use_vocab else self.eos_token] if eos_value is None else [eos_value]
		...
		for x in minibatch:
			t = x[-max_len:] if truncate_first else x[:max_len]
	...
	def build_vocab(self, *args, **kwargs):
		...
		specials = specials + self.additional_special_tokens
	...
	def numericalize(self, arr, device=None):
		...
		if isinstance(arr, tuple):
			...
			for k, v in arr.items():
				ret[k] = self._to_tensor(v, device) if v is not None else None
		else:
...
class TensorField(Field):
	...
	def __init__(self, **kwargs):
		...
		kwargs['eos_token'] = None
		kwargs['additional_special_tokens'] = []
		kwargs['skip_str2index'] = True
		super(TensorField, self).__init__(**kwargs)
	def pad(self, minibatch, pad_value=0):
		if not isinstance(minibatch, list):
			minibatch = list(minibatch)
		if not self.sequential:
			return torch.stack(minibatch)
		max_len = max(x.size(0) for x in minibatch)
		res = minibatch[0].new_full((len(minibatch), max_len, *minibatch[0].size[1:]), pad_value)
