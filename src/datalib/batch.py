...
class Batch(object):
    ...
    def __init__(self, data=None, dataset=None, device=None, ignore_fields=[]):
        """Create a Batch from a list of examples."""
        if data is not None:
            self.batch_size = len(data)
            self.dataset = dataset
            self.fields = list(set(dataset.fields.keys()).difference(set(ignore_fields)))  # copy field names

            for (name, field) in dataset.fields.items():
                if field is not None and name in self.fields:
